...
def getblocks():
    # AABBs
    block = [[-0.6, 0.7, 0, 0.6, 0.8, 1.2],
             [-0.6, -0.8, 0.6, 0.6, -0.7, 2]]
    Obstacles = []
    for i in block:
        i = np.array(i)
...
class env():
    ...
    # def __init__(self, xmin=-5, ymin=0, zmin=-5, xmax=10, ymax=5, zmax=10, resolution=1):  
        ...
        self.balls = getballs()
        self.OBB = np.array([obb([0,0,0],[0,0,0],R_matrix(0,0,0)),
                             obb([0,0,0],[0,0,0],R_matrix(0,0,0))])
        self.start = np.array([0, 0, 0])
        self.goal = np.array([0, 1.1, 0.6])
        self.t = 0 # time 
