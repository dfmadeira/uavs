...
for k = 1:Nsim
    ...
    v_dot = double(subs(p_dot,vars,vals));
    x_dot = [p_dot;v_dot];
...
%% Error State Vector defined as x_til = x - xd
...
A_error = double(subs(A,vars,vals));
[K_error, P_error] = lqr(A_error,B,Q,R);
...
%     p_ref = [Rad*cos(omn*t+phase{iD});Rad*sin(omn*t+phase{iD});(1+dh*(iD-1))*ones(size(t))];
%     v_ref = [-Rad*omn*sin(omn*t+phase{iD});Rad*omn*cos(omn*t+phase{iD});0*ones(size(t))];
%     a_ref = [-Rad*omn^2*cos(omn*t+phase{iD});-Rad*omn^2*sin(omn*t+phase{iD});0*ones(size(t))];
%end
...
%drone_animate(p,p_ref,[-zB_des(2,:);zB_des(1,:);0],t,dcolors);
